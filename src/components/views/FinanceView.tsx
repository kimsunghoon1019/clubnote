...
import { ChevronDown, Download, MoreHorizontal, Plus, Upload, Wallet } from "lucide-react";
...
export function FinanceView() {
  ...
  useEffect(() => {
    ...
    const onPointer = (event: PointerEvent) => {
      ...
      if (target?.closest("[data-export-menu], [data-export-trigger]")) return;
      setExportOpen(false);
    };
    ...
    <>
      <main className="min-h-0 min-w-0 flex-1 overflow-auto [scrollbar-gutter:stable] scrollbar-thin" onClick={dismissInspected}>
        <section className="border-b border-line-soft px-5 py-4 lg:hidden">
          ...
          <p className="mt-1 text-[28px] font-semibold tabular-nums">{formatWon(balance)}</p>
          <div className="mt-4 grid grid-cols-2 gap-3">
            <div>
              <p className="text-compact-caption text-sub">수입</p>
              <p className="mt-0.5 text-compact font-semibold tabular-nums text-up">{formatSignedWon(income)}</p>
            </div>
            <div>
              <p className="text-compact-caption text-sub">지출</p>
              <p className="mt-0.5 text-compact font-semibold tabular-nums text-down">{formatSignedWon(expense)}</p>
            </div>
          </div>
        </section>
        <section className="hidden shrink-0 grid-cols-3 border-b border-line-soft lg:grid">
        ...
        <div className="flex items-center gap-2 border-b border-line-soft px-3 py-2 lg:hidden">
          {PERIODS.map((item) => (
          ...
          <button
            type="button"
            aria-label="거래 추가"
            className="ml-auto flex h-touch w-touch items-center justify-center text-brand-text"
          ...
          <div className="relative">
            ...
            {exportOpen ? (
              ...
              >
                <button type="button" className="flex w-full px-3 py-2 text-left text-compact-caption" onClick={() => void exportLedger()}>
                  내역 내보내기
                </button>
                <button type="button" className="flex w-full px-3 py-2 text-left text-compact-caption" onClick={() => void exportProofs()}>
                  증빙 내보내기
                </button>
                <button
                  type="button"
                  className="flex w-full px-3 py-2 text-left text-compact-caption"
                  onClick={() => {
                    setExportOpen(false);
        ...
        <div className="hidden min-h-12 flex-wrap items-center gap-2 border-b border-line-soft px-5 py-1.5 lg:flex">
          {PERIODS.map((item) => (
            <FilterChip
              key={item}
              active={period === item}
          ...
          <div className="ml-auto flex shrink-0 items-center gap-2">
            ...
            />
            <div ref={exportRef} data-export-trigger className="relative">
              <GhostButton
        ...
        <ul className="lg:hidden">
          {filtered.length === 0 ? (
            <li className="px-5 py-8 text-center text-compact-caption text-faint">조건에 맞는 거래가 없어요.</li>
          ) : (
            filtered.map((row) => (
              <li key={row.id} className="border-b border-line-soft">
                ...
                >
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-compact font-medium">{row.title}</span>
                    <span className="block text-compact-caption text-faint">{formatTxWhen(row.occurredOn, row.occurredAt)}</span>
                  </span>
                  <span
                    className={cn(
                      "shrink-0 tabular-nums text-compact font-semibold",
      ...
      <DetailSurface
        ...
        title={transactions.find((row) => row.id === inspectedTxId)?.title}
